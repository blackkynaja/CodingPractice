...
private func generateAllPossibleSumHelper(allCosts: [[Int]], row: Int, sum: Int, set: inout Set<Int>) {
...
private func generateAllPossibleSum(allCosts: [[Int]]) -> Set<Int> {
    var set = Set<Int>()
    generateAllPossibleSumHelper(allCosts: allCosts, row: 0, sum: 0, set: &set)
    return set
}

private func generatePossibleToppingCost(toppingCosts: [Int]) -> Set<Int>
{
    var allCost = [[Int]]()
